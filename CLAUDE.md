...
`frontend/build/` is **generated, not committed** (gitignored). The backend serves it, but it must be built first.
...
Then restart the backend. Do NOT run `npm start` in the frontend — there is no separate frontend dev server. If `build/` is missing, the backend will print a clear error and exit.

...
The `frontend/build/` directory is **generated, not committed** (it's gitignored). Before the backend can serve the app, the build must exist. Build it once after a fresh clone:

```bash
cd frontend
npm install      # first time only
npm run build    # generates frontend/build/ — required before the backend can serve the app
```

After that, the backend at port 80 serves the static files from `frontend/build/`. Students connect to the NUC's IP directly. If `build/` is missing, the backend will print a clear error and exit.
...
- [ ] `frontend/build/` exists on the NUC (run `cd frontend && npm install && npm run build` once after a fresh clone if not)
